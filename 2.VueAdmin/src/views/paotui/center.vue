				...
				<el-row>
					<el-col :span="12">
						<el-form-item label="跑腿账号" prop="paotuizhanghao">
							<el-input class="list_inp" v-model="user.paotuizhanghao" readonly placeholder="跑腿账号" clearable />
						</el-form-item>
					</el-col>
					<el-col :span="12">
						<el-form-item label="跑腿姓名" prop="paotuixingming">
							<el-input class="list_inp" v-model="user.paotuixingming"  placeholder="跑腿姓名" clearable />
						</el-form-item>
					</el-col>
					<el-col :span="24">
						<el-form-item label="性别" prop="xingbie">
							<el-select 
								...
								placeholder="请选择性别"
								>
								<el-option v-for="item in paotuixingbieLists" :label="item" :value="item"></el-option>
							</el-select>
						</el-form-item>
					</el-col>
					<el-col :span="12">
					...
					<el-col :span="12">
						<el-form-item label="跑腿手机" prop="paotuishouji">
							<el-input class="list_inp" v-model="user.paotuishouji"  placeholder="跑腿手机" clearable />
						</el-form-item>
					</el-col>
					<el-col :span="24">
						<el-form-item label="头像" prop="touxiang">
							<uploads
								...
								@change="paotuitouxiangUploadSuccess">
							</uploads>
						</el-form-item>
					</el-col>
...
<script setup>
	...
	const tableName = ref('paotui')
	const user = ref({})
	const paotuixingbieLists = ref([])
	const init = () => {
		paotuixingbieLists.value = "男,女".split(',')
	}
	const paotuitouxiangUploadSuccess=(fileUrls)=> {
	    user.value.touxiang = fileUrls;
	}
	const onSubmit = () => {
		if((!user.value.paotuizhanghao)){
			context?.$toolUtil.message(`跑腿账号不能为空`,'error')
			return false
		}
		if((!user.value.mima)){
		...
		if((!user.value.paotuixingming)){
			context?.$toolUtil.message(`跑腿姓名不能为空`,'error')
			return false
		}
		...
		if((!user.value.paotuishouji)){
			context?.$toolUtil.message(`跑腿手机不能为空`,'error')
			return false
		}
		if((user.value.paotuishouji)&&(!context?.$toolUtil.isMobile(user.value.paotuishouji))){
			context?.$toolUtil.message(`跑腿手机应输入手机格式`,'error')
			return false
		}
