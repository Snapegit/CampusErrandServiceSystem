...
<template>
	<div>
			...
			<el-table
				...
				<el-table-column prop="ispay" label="支付状态" :resizable='true' :sortable='true' align="left" header-align="left">
					<template #default="scope">
						...
						<el-button v-if="scope.row.ispay!='已支付' && btnAuth('dingdanwancheng','支付')" type="text" size="small" @click.stop="payClick(scope.row)">支付</el-button>
					</template>
				</el-table-column>
				<el-table-column label="操作" width="300" :resizable='true' :sortable='true' align="left" header-align="left">
					<template #default="scope">
						<el-button type="info" v-if=" btnAuth('dingdanwancheng','查看')" @click="infoClick(scope.row.id)">详情</el-button>
						<el-button v-if="btnAuth('dingdanwancheng','评价')" type="success" @click="dingdanpingjiaCrossAddOrUpdateHandler(scope.row,'cross','','[1]','已评价')">评价</el-button>
					</template>
				</el-table-column>
			</el-table>
			<el-pagination 
		...
		<dingdanpingjiaFormModel ref="dingdanpingjiaFormModelRef" @formModelChange="formModelChange"></dingdanpingjiaFormModel>
	</div>
</template>
<script setup>
	...
	import dingdanpingjiaFormModel from '@/views/dingdanpingjia/formModel'
	const dingdanpingjiaFormModelRef = ref(null)
    const dingdanpingjiaCrossAddOrUpdateHandler = (row,type,crossOptAudit,statusColumnName,tips,statusColumnValue) => {
		...
		nextTick(()=>{
			dingdanpingjiaFormModelRef.value.init(row.id,'cross','评价',row,'dingdanwancheng',statusColumnName,tips,statusColumnValue)
		})
    }
	//初始化
	const init = () => {
